...
public class CreateTranslator extends Translator implements TranslatorInterface {
	// similar to DEFINE translator.
	// DEFINE XXX AS NUMBER
	// CREATE NUMBER XXX
    ...
    public CreateTranslator(int lineNum, Line line, ArrayList<String> preLines) {
    ...
    public void translateTo(String language) {
        ...
        if (this.line.startsWith("CREATE A CONSTANT")) {
            // CREATE A CONSTANT NUMBER variable = value
            System.out.println(">>>Define a constant...");

            if (this.line.component(6).equals("=")) {

                String vName = this.line.component(5).trim();
                String vDatatype = this.line.component(4).trim();
                String vValue = this.line.component(this.line.componentOf("=") + 1, -1).trim();
                ...
                super.reportError("Constant name is illegal or wrong define type in create keyword");
            }

        } else if (this.line.startsWith("CREATE ARRAY")) {
            // CREATE ARRAY NUMBER array[i..j]
            System.out.println(">>>Define an array...");

            String arrayName = this.line.component(4);
            if (arrayName.contains("[1..") && arrayName.endsWith("]") && !arrayName.contains("][1..")) {
                // Define an array but don't assign value.

                if (this.line.component(5).equals(Line.ERROR_MSG)) {
                    ...
                    if (index == 0) {
                        ...
                        String aDataType = this.line.component(3).trim();
                        String aLength = "";
                        try {
            ...
            } else if (!arrayName.contains("[1..") && !arrayName.endsWith("]") && !arrayName.contains("][1..") && this.line.component(5).equals("=")) {
                // Define an array and assign values.
                System.out.println(">>>Define an array... and assign values");

                if (this.line.component(6).startsWith("[") && this.line.component(-1).endsWith("]")
                        && !this.line.component(6).startsWith("[[") && !this.line.component(-1).endsWith("]]")) {
                ...
                } else if (this.line.component(6).startsWith("[[") && this.line.component(-1).endsWith("]]")) {
                    System.out.println(">>>2D array");
                    int index1 = 0, index2 = 0;
                    for (int i = 0; i < this.line.getPureContent().length(); i++) {
                        if (i == this.line.getPureContent().length() - 2) {
                            if (this.line.getPureContent().charAt(i) == ']' && this.line.getPureContent().charAt(i + 1) == ']') {
                                index2 = i;
                            } else {
                                super.reportError("Illegal array define exist");
                            }
        ...
        } else if (this.line.startsWith("CREATE") && !this.line.startsWith("CREATE ARRAY") && !this.line.startsWith("CREATE A CONSTANT")) {
            // CREATE NUMBER variable = value
            System.out.println(">>>Define a variable...");

            if (this.line.component(4).equals("=")) {

                if (this.line.component(3).contains(",")) {
                    // Define multiple variables at same time.

                    String vNames[] = this.line.component(3).split(",");
                    for (String s: vNames) {
                        String vName = s.trim();
                        String vDatatype = this.line.component(2).trim();
                        String vValue = this.line.component(this.line.componentOf("=") + 1, -1).trim();
                ...
                } else if (!this.line.component(3).contains(",")) {
                    // Define a variable merely.

                    String vName = this.line.component(3).trim();
                    String vDatatype = this.line.component(2).trim();
                    String vValue = this.line.component(this.line.componentOf("=") + 1, -1).trim();
                    Variable v = new Variable(vName, vDatatype, vValue, false);
                ...
                if (this.line.component(4).equals(Line.ERROR_MSG)) {
                ...
                super.reportError("Variable name is illegal or wrong define type in create keyword");
            }

        } else {
            super.reportError("Variable expression with define keyword");
        }
    }
